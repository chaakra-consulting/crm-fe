<template>
        ...
        <div>
          ...
          <nav aria-label="breadcrumb">
            <ol class="flex flex-wrap space-x-1 text-sm text-gray-500">
              ...
              <li class="text-dark font-medium" aria-current="page">Tags</li>
            </ol>
          </nav>
        </div>
                  ...
                  >
                    <template #bodyCell="{ column, record, index }">
                      ...
                      <template v-if="column.key === 'color'">
                        <span class="inline-flex items-center" :class="record.color">
                          {{ record.color_text }}
  ...
  <component
    v-if="TagsCreateModal && showCreateModal"
    :is="TagsCreateModal"
    :isOpen="showCreateModal"
    @close="closeCreateModal"
    @refresh-list="fetchTags"
  />

  <component
    v-if="TagsEditModal && showEditModal"
    :is="TagsEditModal"
...
const columns = [
  ...
  },
  {
    ...
    width: 100,
    sorter: {
      compare: (a, b) => (a.color.toLowerCase() > b.color.toLowerCase() ? -1 : 1),
    },
  },
  {
    title: 'Aksi',
...
export default {
  ...
  computed: {
    filteredPages() {
      ...
      return this.data.filter((record) => {
        return record.name && record.name.toLowerCase().includes(query)
      })
    },
    paginatedData() {
  ...
  methods: {
    ...
    async openCreateModal() {
      if (!this.TagsCreateModal) {
        const module = await import('./components/tags-create-modal.vue')
        this.TagsCreateModal = module.default
      }
    ...
    async openEditModal(record) {
      this.selectedTag = record
      if (!this.TagsEditModal) {
        const module = await import('./components/tags-edit-modal.vue')
        this.TagsEditModal = module.default
      }
