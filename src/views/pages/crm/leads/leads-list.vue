<template>
                  ...
                  <a-table
                    ...
                    <template #bodyCell="{ column, record, index }">
                      ...
                      <template v-if="column.key === 'company_name'">
                        <h6 class="flex items-center text-[14px] font-medium mb-0">
                          <router-link :to="'/crm/company-details'" class="flex flex-col">
                            {{ record.company_name }}
                            <span class="truncate w-32 text-default text-[13px] mt-1 font-normal">{{
                      ...
                      <template v-if="column.key === 'phone_number'">
                        <a :href="'https://wa.me/' + record.phone_number" target="_blank">
                          {{ record.phone_number }}</a>
                      </template>
                      <template v-if="column.key === 'status'">
                        <span class="px-1.5 py-0.5 rounded text-[12px] text-white inline-flex" :class="{
                          'bg-success': record.status === 'Qualified',
                          'bg-info': record.status === 'New',
                          'bg-warning': record.status === 'Warm',
                          'bg-danger': record.status === 'Unqualified',
                        }">
                          {{ record.status }}
                        </span>
                      </template>
  ...
  <component v-if="EditModal && showEditModal" :is="EditModal" :lead="selectedLead" :isOpen="showEditModal"
...
const columns = [
  ...
  },
  {
    ...
    key: 'company_name',
    sorter: {
      compare: (a, b) => (a.company_name.toLowerCase() > b.company_name.toLowerCase() ? -1 : 1),
    },
  },
  {
    title: 'Nomor Telepon',
    ...
    key: 'phone_number',
    sorter: {
      compare: (a, b) => (a.phone_number.toLowerCase() > b.phone_number.toLowerCase() ? -1 : 1),
    },
  },
  {
    title: 'Status',
  ...
  },
  {
    ...
    key: 'created_at',
    sorter: {
      compare: (a, b) => (a.created_at.toLowerCase() > b.created_at.toLowerCase() ? -1 : 1),
    },
  },
  {
    title: 'Aksi',
...
export default {
  ...
  computed: {
    filteredPages() {
      ...
      return this.leads_data.filter((record) => {
        return (
  ...
  methods: {
    async openCreateModal() {
      if (!this.CreateModal) {
        ...
        this.CreateModal = module.default
      }
    ...
    async fetchData() {
      ...
      try {
        ...
        this.leads_data = projects.map((item) => {
