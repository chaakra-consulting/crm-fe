    ...
    editData() {
      ...
      if (!this.company || !this.company.id) {
        this.$swal({
          icon: 'error',
          title: 'Gagal!',
          text: 'ID Company tidak ditemukan!',
        })
        return
      }
      ...
      api
        .post(`/companies/update/${this.company.id}`, formData, {
          headers: {
    ...
    company: {
      handler(newVal) {
        if (newVal) {
          ...
          this.form.jenis = newVal.jenis ?? ''
          this.form.bentuk = newVal.bentuk ?? ''
          this.form.npwp = newVal.npwp ?? ''
          this.form.province_id = newVal.province_id ?? ''
          this.form.city_id = newVal.city_id ?? ''
          this.form.pic_contact_id = newVal.pic_contact_id ? String(newVal.pic_contact_id) : ''
