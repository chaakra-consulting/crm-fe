        ...
        <li>
          <ul class="mb-[25px]">
            <template v-for="menu in item.menu" :key="menu.menuValue">
              <li v-if="!menu.hasSubRoute" :class="{ active: isMenuActive(menu) }" class="mb-[5px]">
                <router-link
                  v-if="menu.route"
                  :to="menu.route"
  ...
  computed: {
    isMenuActive() {
      return (menu) => {
        ...
        return links.some((link) => current === link || current.startsWith(link + '/'))
      }
    },
    isActive() {
      return (menu) => {
    ...
    isSubActive() {
      return (subMenu) => {
        ...
        if (
          subMenu.route &&
          (currentPath === subMenu.route || currentPath.startsWith(subMenu.route + '/'))
        )
          return true
        ...
        if (subMenu.subMenusTwo && subMenu.subMenusTwo.length > 0) {
          return subMenu.subMenusTwo.some((childSubMenu) => {
            if (
              childSubMenu.route &&
              (currentPath === childSubMenu.route ||
                currentPath.startsWith(childSubMenu.route + '/'))
            )
              return true
            if (childSubMenu.active_link) {
              return (
                currentPath === childSubMenu.active_link ||
            ...
            newGroup.menu = newGroup.menu
              ...
              .map((menu) => {
                ...
                if (menu.subMenusTwo && menu.subMenusTwo.length) {
                  menu.subMenusTwo = menu.subMenusTwo.filter(
                    (sub2) => !sub2.roles || sub2.roles.includes(role),
                  )
                }
                return menu
              })
