<template>
  <!-- Sidebar -->
  <div
    class="sidebar fixed top-0 left-0 bottom-0 w-sidebar bg-white border-r border-borderColor"
    id="sidebar"
    :class="{ open: isSidebarOpen }"
  >
    <!-- Logo -->
    <div
      class="sidebar-logo fixed h-[56px] w-sidebar pt-[19px] px-4 border-b border-borderColor"
    >
      <div>
      ...
      <button
        id="toggle_btn"
        @click="toggleSidebar"
        class="sidenav-toggle-btn btn p-0 active absolute top-4 right-2 w-[22px] h-[22px] rounded-full text-dark text-[16px]"
      >
        <i class="ti ti-arrow-bar-to-left"></i>
      </button>
      <!-- Sidebar Menu Close -->
      <button
        class="sidebar-close text-[20px] absolute top-1/2 -translate-y-1/2 right-[20px] rounded-full"
        @click="closeSidebar"
      >
